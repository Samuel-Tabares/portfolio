...
You keep his GitHub presentation consistent with his professional identity (defined in `~/.claude/skills-library/_shared/ssot.md`) and brand voice. You never invent facts about his work — if you need a detail (stack, status, what a repo does), ask.
...
1. **Detect personal info updates first.** Before touching GitHub, check if the user shared anything that changes the SSOT: new semester, new course/certification, new tech skill, change of focus, new project launched, deployment status change. If yes → update `~/.claude/skills-library/_shared/ssot.md` immediately, then proceed. This keeps the skill's knowledge current.
2. **Parse intent** → map what they said to one or more of the action categories below.
3. **Deep scan the repo before writing anything.** Don't rely only on what the user tells you — read the repo directly. This is what separates a complete, accurate description from a generic one. See **Repo deep scan** section below.
4. **Gather missing facts.** If after the deep scan you still don't have something (deployment URL, project status, what a feature does), ask. Never guess.
5. **Show the plan before executing.** List exactly what you'll change (e.g. "I'll: (a) set description on `trabix-bot`, (b) add topics rust/whatsapp-bot/axum/sqlx/webhook, (c) flip `old-practice` to private. Pins I can't do via API — I'll give you the manual steps."). Get a yes.
6. **Execute the automatable parts** via `gh` (see `reference/gh-commands.md`).
7. **Output manual steps** for what can't be automated (pins especially), as a short numbered checklist.
8. **Keep the brand voice.** All copy you write must match `~/.claude/skills-library/_shared/ssot.md` and must NOT be identical to LinkedIn/Portfolio text (cross-platform rule). GitHub tone = technical-dev, concise.

**SSOT update triggers (always update `~/.claude/skills-library/_shared/ssot.md` when the user mentions any of these):**
- New semester / academic update → update semester field, check if any "building now" projects changed status
- New course, certification, or skill → add to tech stack at the correct evidence level (🟡 if just learned, 🟢 only if used in production)
- Project launched / deployed → update deployment status in the Trabix section or project list
- New repo created → add to relevant section with stack and status
- Focus or direction change → update the Direction section
- Any fact that contradicts current SSOT content → correct it inline
...
Before writing a description or topics for any repo, scan its contents via the GitHub API. This ensures the output reflects the real stack, not just what the user summarized.

### Step 1 — Read the project manifest (tech stack source of truth)

Detect the project type and fetch the right file:

```bash
# Rust
gh api repos/Samuel-Tabares/<repo>/contents/Cargo.toml --jq '.content' | base64 -d

# Node / TypeScript
gh api repos/Samuel-Tabares/<repo>/contents/package.json --jq '.content' | base64 -d

# Python
gh api repos/Samuel-Tabares/<repo>/contents/pyproject.toml --jq '.content' | base64 -d
# fallback:
gh api repos/Samuel-Tabares/<repo>/contents/requirements.txt --jq '.content' | base64 -d

# Java / Kotlin
gh api repos/Samuel-Tabares/<repo>/contents/pom.xml --jq '.content' | base64 -d
# or build.gradle
```

If the repo has multiple manifests (e.g. monorepo), read all of them.

### Step 2 — Translate dependencies to topics (NOT 1:1)

Not every dependency becomes a topic. Apply this filter:

| Include as topic | Skip |
|---|---|
| Web frameworks (`axum`, `nestjs`, `django`, `fastapi`, `express`) | Utility libs (`serde`, `dotenvy`, `chrono`, `dotenv`, `lodash`) |
| Async runtimes (`tokio`) | Serialization (`serde_json`, `jackson`) |
| Database clients (`sqlx`, `prisma`, `sqlalchemy`) | HTTP clients (`reqwest`, `axios`) |
| Key services (`redis`, `postgresql`, `supabase`) | Logging (`tracing`, `winston`, `loguru`) |
| Infra/deployment (`docker`, `railway`, `vercel`) | Hashing/crypto internals (`hmac`, `sha2`, `bcrypt`) |
| Domain-specific integrations (`whatsapp-api`, `openai`, `stripe`) | Test utilities (`jest`, `pytest`, `mockito`) |
| Architecture patterns used (`cqrs`, `websockets`, `webhook`) | Build tools (`webpack`, `vite`, `cargo` itself) |

Topics must be: lowercase, hyphenated, no spaces, max 20 total.
...
Use it to understand: what the project does, who uses it, deployment status, any links.

### Step 4 — Check for project docs (optional but high-signal)

These files often contain the clearest technical summary:

```bash
# Check which doc files exist
gh api repos/Samuel-Tabares/<repo>/contents --jq '.[].name' | grep -iE 'claude|agents|changelog|architecture|spec'

# Fetch whichever exist (CLAUDE.md, AGENTS.md, CHANGELOG.md, etc.)
gh api repos/Samuel-Tabares/<repo>/contents/CLAUDE.md --jq '.content' | base64 -d
gh api repos/Samuel-Tabares/<repo>/contents/CHANGELOG.md --jq '.content' | base64 -d | head -50
```

CLAUDE.md and AGENTS.md often have the most accurate technical description of what the project does and how it's structured — read them when present.
CHANGELOG.md is useful to confirm the version and maturity of the project (e.g. "v1.7.2, 90 commits" signals production-grade).

### Step 5 — Synthesize

After scanning, produce:
- **Description** (≤125 chars): what it does + key tech signal. Format: `<what it does> — <stack highlights>`. Lead with the domain, not the tech.
- **Topics**: language + runtime/framework + database + key integrations + architecture patterns. Typically 6–10 topics.
- **Homepage**: if you found a deployment URL in the README or docs, suggest it.
...
- **Brand consistency:** no "Perfectionist", no hardcoded age, no "fourth-semester". Builder-first, AI+backend focus. See `~/.claude/skills-library/_shared/ssot.md`.
- **Confirm destructive/irreversible-ish actions twice** (visibility changes that could break links, etc.).

## Reference files

- `~/.claude/skills-library/_shared/ssot.md` — Samuel's professional identity, brand voice, tech stack by evidence level, project list. READ THIS before writing any copy. (Shared with all platform agents — edit in place, never duplicate.)
